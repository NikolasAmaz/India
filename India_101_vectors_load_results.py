...
for nod in Nodes [ :1  ] :
  for lay in Layers [  -1: ]:
    for iters in Iterations [  -1: ] :
      for batch_size in Batch_sizes [ :1 ] :      
             
            model = model_from_json(open( mypath + 'Hyderabad' +'_' +  Approach + '_' +  Method + '_' + str(lay) + '_' + str(nod) + '_' + Activations  \
                 + '_' + str(iters) + '_' + str(batch_size) +'.json' ).read())
            model.load_weights(mypath + 'Hyderabad' + Approach + '_' +  Method + '_' + str(lay) + '_' + str(nod) + '_' + Activations  \
                 + '_' + str(iters) +'.h5' )
            ...
            abs_err = np.sqrt( abs_err/float(len(pred)) )
            ...
            RESULTS.append([ [ Approach , nod , lay , iters , batch_size ] , abs_err , rel_err ] ) 
